                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group mb-3" id="branchSelection" style="display: none;">
                                    ...
                                    <select class="form-control @error('branch_market_id') is-invalid @enderror" 
                                        ...
                                        <option value="">Select Market</option>
                                        @foreach($markets as $market)
                                            <option value="{{ $market->id }}" {{ old('branch_market_id') == $market->id ? 'selected' : '' }}>
                                                {{ $market->name }}
                                            </option>
                                        @endforeach
                                    </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleSelections() {
        ...
        console.log('Market Admin type:', '{{ \App\Models\User::TYPE_MARKET_ADMIN }}');
        console.log('Branch Admin type:', '{{ \App\Models\User::TYPE_BRANCH_ADMIN }}');
        
        if (selectedType === '{{ \App\Models\User::TYPE_MARKET_ADMIN }}') {
            console.log('Showing market selection');
            ...
            console.log('Hiding both selections');
            marketSelection.style.display = 'none';
            branchSelection.style.display = 'none';
        }
    }
            ...
            fetch(url)
                ...
                .then(branches => {
                    ...
                    if (!Array.isArray(branches)) {
                        console.error('Invalid response format:', branches);
    ...
    // Run on page load
    console.log('Running toggleSelections on page load');
    toggleSelections();

    // If branch market is pre-selected, trigger the change event
    if (branchMarketSelect.value) {
        console.log('Branch market pre-selected, triggering change event');
